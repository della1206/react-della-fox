    ...
    try {
      ...
      if (result.length > 0) {
        ...
        if (userObj.password === dataForm.password) {
          ...
          if (onLoginSuccess) {
            onLoginSuccess(userObj.nama);
          }

          // SEMUA USER LANGSUNG KE DASHBOARD ADMIN
        ...
        alert("Maaf, Username/Email tidak ditemukan di database!\n\nSilakan daftar akun terlebih dahulu.");
      }
    } catch (err) {
      ...
      <div className="w-full max-w-md bg-white/80 backdrop-blur-md rounded-3xl p-8 md:p-10 shadow-2xl border border-blue-50 relative z-10 animate-in fade-in zoom-in-95 duration-500">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 text-xs mt-2 font-medium">
            Selamat datang kembali. Silakan masuk ke dashboard admin.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          >
            {loading ? "Mohon Tunggu..." : "Masuk Dashboard →"}
          </button>
        </form>

        {/* LINK REGISTRASI - DAFTAR AKUN BARU */}
